...
export function TimerScreen({
  ...
  const toggleRun = useCallback(() => {
    ...
    // и возвращать его самим было бы спором с человеком.
    autoRaise.current();
  ...
  /**
   * Автоподъём щита на старте фокуса.
   *
   * Живёт в ref, а не в зависимостях `toggleRun`: старт и без того
   * пересоздаётся на каждый тик остатка, а тащить туда ещё и настройки
   * с выбором списка значило бы пересобирать его вдвое чаще без всякой
   * пользы.
   *
   * Разрешения здесь не просим. Если Экранное время отозвано, щит просто
   * не поднимется — а сказать об этом должен экран «Погружение», где для
